...
export const migration001Init: Migration = {
  ...
  up: async (db: SQLiteDatabase) => {
    ...
    await db.execAsync(`CREATE INDEX IF NOT EXISTS idx_meta_pilar ON meta(pilar_id)`);
    ...
    await db.execAsync(`CREATE INDEX IF NOT EXISTS idx_execucao_meta ON execucao(meta_id)`);
  ...
  down: async (db: SQLiteDatabase) => {
    ...
    await db.execAsync('DROP TABLE IF EXISTS compromisso');
    await db.execAsync('DROP TABLE IF EXISTS investimento');
    await db.execAsync('DROP TABLE IF EXISTS lancamento_financeiro');
    await db.execAsync('DROP TABLE IF EXISTS execucao');
    await db.execAsync('DROP TABLE IF EXISTS meta');
    await db.execAsync('DROP TABLE IF EXISTS pilar');
      ...
      if (migration.version > targetVersion && migration.version <= currentVersion) {
        if (migration.down) {
          try {
            ...
            await this.db.execAsync('DELETE FROM schema_version WHERE version = ?', [
              migration.version,
            ]);
